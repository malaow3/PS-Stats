...
func updateRating(cfg *config.Config, battle_inst *Battle) error {
...
func uploadBattle(cfg *config.Config, battle_inst *Battle) error {
	if battle_inst == nil {
	...
	// if the total number of pokemon is less than 6, add random as the rest
	if len(strings.Split(your_team_string, ",")) < 6 {
		for i := len(strings.Split(your_team_string, ",")); i < 6; i++ {
			your_team_string += ",random"
		}
	}
